...
# ============== IRIS v1.1 - GIRL VOICE + BROWSER YOUTUBE ==============
MODEL = "gemma4:e4b"
WAKE_WORD = "hey iris"
...
# === FEMALE VOICE (espeak-ng) ===
voices = engine.getProperty('voices')
# Try these female voices one by one until you like it:
# english+f1, english+f2, english+f3, english+f4
pipeline = KPipeline(lang_code='a')
engine.setProperty('voice', 'english+f3')   # Change to f1/f2/f4 if you want different tone

print("Loading Whisper model...")
whisper_model = WhisperModel("small", device="cpu", compute_type="int8")

print(f"🛠️  Iris v1.1 online — Gemma 4 ({MODEL}) + **Girl Voice**")
...
def get_system_info():
    ...
    battery_str = f"{battery.percent}% {'charging' if battery.power_plugged else 'on battery'}" if battery else "unknown"
    speak(f"Current time is {time.strftime('%I:%M %p')}. Battery is at {battery_str}. CPU usage is {psutil.cpu_percent()}%.")
...
def execute_command(command):
    try:
        if "play" in command or "youtube" in command or "song" in command:
            ...
            response = ollama.chat(model=MODEL, messages=[{'role': 'user', 'content': f"You are Iris. {command}"}])
